  ...
  const handleDownload = async (batchId: number, quantity: number) => {
    ...
    try {
      ...
      for (const code of qrCodes) {
        ...
        doc.setFontSize(8)
        doc.text(code.qrCodeId.substring(0, 13), x, y + qrSize + 4, { maxWidth: qrSize })
      ...
      const date = batch ? new Date(batch.generatedAt).toISOString().split('T')[0] : 'unknown'
            ...
            {batches.length === 0 ? (
              <tr>
                <td colSpan={6} className="px-6 py-8 text-center text-gray-500">
                  No QR code batches found. Generate your first batch to get started.
                </td>
              </tr>
            ) : (
              batches.map((batch) => (
